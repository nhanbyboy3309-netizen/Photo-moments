...
const DEFAULT_SETTINGS: SiteSettings = {
  ...
  photoPrice: 50000,
  shippingFee: 30000, // New default shipping fee
  logoType: 'text',
...
export const confirmPhotoPayment = async (photoId: string, amount: number, customerEmail: string): Promise<void> => {
  // Khách xác nhận đã chuyển khoản mua ảnh lẻ: báo lại cho Admin ngay.
  try {
    const appUrl = window.location.origin + window.location.pathname;
    await apiCall('notify_admin', {
      id: photoId,
      amount: amount,
      customer: customerEmail || 'Khách hàng',
      items: [],
    ...
    console.warn('[confirmPhotoPayment] Không thể gửi thông báo cho Admin:', e);
  }
};

export const confirmOrderPayment = async (orderId: string, amount: number, customerEmail: string): Promise<void> => {
  // Khách xác nhận đã chuyển khoản: báo lại cho Admin ngay để không phải tự kiểm tra tab Giao dịch.
  try {
    const order = await getOrderById(orderId);
    const appUrl = window.location.origin + window.location.pathname;
    await apiCall('notify_admin', {
    ...
    console.warn('[confirmOrderPayment] Không thể gửi thông báo cho Admin:', e);
  }
};
