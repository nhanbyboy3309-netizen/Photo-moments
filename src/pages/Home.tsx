...
const Home: React.FC = () => {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    // 1. Thử tìm trong DB của photo-moments trước.
    const photo = await getPhotoById(targetId);
    if (photo) {
        setLoading(false);
        navigate(`/view/${targetId}`);
        return;
    ...
    // 3. Cuối cùng, thử chuyển hướng sang ID Photo Booth Pro (database ảnh chụp thẻ riêng).
    setLoading(false);
    window.location.href = `${PHOTO_BOOTH_PRO_URL}/?photoId=${encodeURIComponent(targetId)}`;
  };
