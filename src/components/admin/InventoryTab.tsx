...
import { Package, Search, X, Printer, History, Square, CheckSquare, Grid, Loader2, Activity, ArrowDownCircle, ArrowUpCircle } from 'lucide-react';
import { Product, InventoryLog, SiteSettings } from '../../types';
import { getProducts, getInventoryLogs, getSettings } from '../../services/mockBackend';
    ...
    <div className="space-y-6 animate-fade-in no-print">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 border-b border-zinc-800 pb-2">
            <div className="flex gap-4">
                ...
                <button onClick={() => setActiveSubTab('history')} className={`pb-2 px-2 text-sm font-bold border-b-2 transition-colors flex items-center gap-1 ${activeSubTab === 'history' ? 'border-blue-500 text-white' : 'border-transparent text-zinc-500'}`}><History className="w-3.5 h-3.5" /> Lịch Sử</button>
            </div>
            <div className="flex gap-2 w-full md:w-auto">
                <div className="relative flex-1 md:flex-none">
                    ...
                    <input
                        ...
                        placeholder={activeSubTab === 'stock' ? 'Tìm mã hoặc tên sản phẩm...' : 'Tìm theo mã sản phẩm...'}
                        className="w-full md:w-64 bg-zinc-900 border border-zinc-800 rounded-lg pl-9 pr-3 py-2 text-sm text-white placeholder-zinc-600 focus:border-blue-500 outline-none"
                    />
                </div>
                {activeSubTab === 'stock' && (
                    <>
                        <button onClick={() => handleBulkPrint('selected')} disabled={selectedIds.size === 0} className="bg-zinc-800 hover:bg-zinc-700 disabled:opacity-50 text-white px-4 py-2 rounded-lg text-xs font-bold flex items-center gap-2 whitespace-nowrap">In {selectedIds.size} tem</button>
                        <button onClick={() => handleBulkPrint('all')} className="bg-zinc-800 hover:bg-zinc-700 text-white px-4 py-2 rounded-lg text-xs font-bold flex items-center gap-2 whitespace-nowrap"><Grid className="w-3 h-3" /> In tất cả</button>
                    </>
                )}
            </div>
        </div>
        ...
        {loading ? (
            <div className="flex justify-center py-16"><Loader2 className="w-8 h-8 text-zinc-600 animate-spin" /></div>
        ) : activeSubTab === 'stock' ? (
                 ...
                 <table className="w-full text-left text-sm">
                    <thead className="bg-zinc-900/80 text-zinc-500 uppercase font-black text-[10px] tracking-widest">
                        <tr>
                            ...
                            <th className="p-4">Tồn Sau</th>
                            <th className="p-4">Ghi Chú</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-zinc-800/50">
                        {logs.filter(l => l.productId.includes(search) || l.productName.toLowerCase().includes(search.toLowerCase())).length === 0 ? (
                            <tr><td colSpan={6} className="p-8 text-center text-zinc-600">Chưa có lịch sử thay đổi kho.</td></tr>
                        ) : logs.filter(l => l.productId.includes(search) || l.productName.toLowerCase().includes(search.toLowerCase())).map(log => (
                            <tr key={log.id} className="hover:bg-zinc-800/30 transition-colors">
                                <td className="p-4 text-zinc-500 whitespace-nowrap">{new Date(log.createdAt).toLocaleString('vi-VN')}</td>
                                <td className="p-4 font-mono text-zinc-400">{log.productId}</td>
                                <td className="p-4 font-bold text-white">{log.productName}</td>
                                <td className={`p-4 font-mono font-bold flex items-center gap-1 ${log.changeAmount >= 0 ? 'text-green-500' : 'text-red-500'}`}>
                                    {log.changeAmount >= 0 ? <ArrowUpCircle className="w-3.5 h-3.5" /> : <ArrowDownCircle className="w-3.5 h-3.5" />}
                                    {log.changeAmount >= 0 ? `+${log.changeAmount}` : log.changeAmount}
                                </td>
                                <td className="p-4"><span className="bg-zinc-800 px-3 py-1 rounded-full font-mono text-xs">{log.currentStock}</span></td>
                                <td className="p-4 text-zinc-500">{log.note || '-'}</td>
                            </tr>
                        ))}
                    </tbody>
                 </table>
