...
function getSheetData(sheetName) {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    for (let j = 0; j < headers.length; j++) {
      ...
      // Orders sheet stores phone/email under short header names; expose them
      // as customerPhone/customerEmail to match what the app expects.
      if (sheetName === 'Orders' && key === 'phone') key = 'customerPhone';
      if (sheetName === 'Orders' && key === 'email') key = 'customerEmail';
      obj[key] = val;
    }
    data.push(obj);
  }
...
function saveRow(sheetName, itemObj) {
  ...
  for (let header of headers) {
    ...
    // Orders sheet stores phone/email under short header names, but the app
    // sends them as customerPhone/customerEmail — map them so they don't get lost.
    if (header === 'phone' && itemObj.customerPhone !== undefined) val = itemObj.customerPhone;
    if (header === 'email' && itemObj.customerEmail !== undefined) val = itemObj.customerEmail;
    // Force phone numbers to be stored as text so Sheets doesn't strip the leading 0.
    if (header === 'phone' && val && String(val).charAt(0) !== "'") val = "'" + val;
  ...
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][idColIndex]) === String(id)) {
      for (let key in updates) {
        ...
        if (sheetName === 'Orders' && key === 'customerPhone') colName = 'phone';
        if (sheetName === 'Orders' && key === 'customerEmail') colName = 'email';
...
function sendAdminNotification(data) {
  try {
    ...
    // data.reason: 'new_order' (default) hoặc 'payment_confirmed' — dùng để phân biệt
    // email "đơn mới tạo" với email "khách xác nhận đã chuyển khoản", tránh gửi 2 email
    // trùng nội dung liên tiếp khi khách bấm xác nhận ngay sau khi đặt hàng.
    const isPaymentConfirm = data.reason === 'payment_confirmed';
    const introText = isPaymentConfirm
      ? 'Khách hàng vừa bấm xác nhận đã chuyển khoản cho đơn hàng dưới đây. Vui lòng kiểm tra và duyệt.'
      : 'Hệ thống vừa ghi nhận đơn hàng mới.';
    const emailTitle = isPaymentConfirm
      ? `Khách Xác Nhận Chuyển Khoản: ${data.id}`
      : `Đơn Hàng Mới: ${data.id}`;
    const subjectPrefix = isPaymentConfirm ? '[Xác nhận CK]' : '[New Order]';
    ...
    const body = renderEmailTemplate(emailTitle, content, actions);

    MailApp.sendEmail({
      to: adminEmail,
      subject: `${subjectPrefix} ${data.id} - ${formatCurrency(data.amount)}`,
      htmlBody: body,
